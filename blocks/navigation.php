<div class="menu">
  <div class="content">
    <div class="row row-jcsb row-ai-center">
      <div class="wrapper-img wrapper-header-all-product-logo"><a href="/"><img src="img/mini-logo.svg" alt=""></a></div>
      <span class="menu__button menu-toggle"></span>
    </div>
    <div class="menu__body">
      <div class="row row-jcsb nav__row">
        <nav class="nav">
          <ul class="nav__ul">
            <li class="nav__li"><a href="all-product.php">Shop</a></li>
            <li class="nav__li"><a href="delivery.php">Delivery</a></li>
            <li class="nav__li"><a href="#">Catalog</a></li>
            <li class="nav__li"><a href="designer.php">Team</a></li>
            <li class="nav__li"><a href="designer.php">Contact</a></li>
          </ul>
        </nav>
        <span class="nav__li nav__li-col not_available"><a href="#">virtual showroom</a></span>
      </div>
      <div class="row row-jcsb menu__social">
        <div class="copyright"> &#169; <?= date('Y'); ?> Furn. All rights reserved. Ukraine, Odessa</div>
        <ul class="row menu__social-ul">
          <li class="menu__social-li"><a href="https://wa.me/message/6F3P4J5BBHEZA1" target="_blank">Whatsapp</a></li>
          <li class="menu__social-li"><a href="https://www.instagram.com/furn_furn_furn/" target="_blank">Instagram</a> </li>
          <li class="menu__social-li"><a href="https://www.behance.net/furnmanufacture" target="_blank">Behance</a></li>
          <li class="menu__social-li"><a href="tel:+380 96 191 8177" target="_blank">Telegram</a></li>
          <li class="menu__social-li"><a href="https://www.pinterest.com/fmanufacture/" target="_blank">Pinterest</a></li>
        </ul>
      </div>
    </div>
  </div>
</div>